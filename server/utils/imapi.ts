...
export class IMAPI {
  ...
  public async getEntityChildren(
    sessionId: string,
    iri: string,
    schemeIris?: any,
  ) {
  ...
  public async findRequestMissingArguments(
    sessionId: string,
    ip: string,
    queryRequest: QueryRequest,
  ): Promise<ArgumentReference[]> {
    return (await $fetch(
      `${useRuntimeConfig().public.imapiUrl}query/protected/findRequestMissingArguments`,
      {
  ...
  public async getQueryFromIri(
    sessionId: string,
    queryIri: string,
  ): Promise<Query> {
    return (await $fetch(
      `${useRuntimeConfig().public.imapiUrl}query/protected/queryFromIri`,
      {
        headers: { cookie: `session_id=${sessionId}` },
        params: {
          queryIri: queryIri,
