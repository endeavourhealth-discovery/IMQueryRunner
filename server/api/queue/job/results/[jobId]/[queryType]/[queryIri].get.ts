...
import { IMQ, IMQType, QUERY } from "~~/models/AutoGen";
...
export default defineEventHandler(async (event) => {
  const { jobId, queryIri, queryType } = await getValidatedRouterParams(
    event,
    paramSchema.parse,
  );
  ...
  const queryResultSet = await mysqlDb.query.queryResultSetTable.findFirst({
    where: (q) =>
      and(eq(q.jobId, Number(jobId)), eq(q.queryIri, decodedQueryIri)),
  });
  if (!queryResultSet) {
  ...
  if (queryType === IMQType.INDICATOR) {
    const indicatorResult = await mysqlDb.query.indicatorResultTable.findFirst({
      where: (q) =>
        and(
          eq(q.queryIri, decodedQueryIri),
          eq(q.queryResultSetId, queryResultSet.id),
        ),
    });
    ...
    if (queryType === IMQType.COHORT) {
      ...
      const totalResult = await mysqlDb
        .select({ count: count() })
