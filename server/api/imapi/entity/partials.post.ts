    ...
    requestBody: {
      ...
      content: {
        "application/json": {
          schema: {
            type: "object",
            summary: "Request allowing for multiple getPartial requests at once",
            properties: {
              typeIris: {
                type: "array",
                items: {
...
export default defineEventHandler(async (event): Promise<any> => {
  ...
  const { typeIris, predicates } = await readValidatedBody(event, bodySchema.parse);
  return await EntityService.getPartialEntities(sessionId, typeIris, predicates);
});
