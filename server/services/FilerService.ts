...
const FilerService = {
  async moveFolder(sessionId: string, entity: string, oldFolder: string, newFolder: string): Promise<void> {
  ...
  async addToFolder(sessionId: string, entity: string, folder: string): Promise<void> {
  ...
  async createFolder(sessionId: string, container: string, name: string): Promise<string> {
  ...
  async fileDocument(sessionId: string, document: TTDocument): Promise<{ [x: string]: string }> {
  ...
  async getTaskProgress(sessionId: string, taskId: string): Promise<{ [x: string]: number }> {
    return await $fetch<{ [x: string]: number }>(API_URL + `/file/document/${taskId}`, {
      headers: { cookie: `session_id=${sessionId}` },
      method: "GET"
    });
  }
