...
async function getLatestReleaseNotes() {
  ...
  const release = await GithubService.getLatestRelease(REPO.IM_QUERY_RUNNER);
...
async function getAllRepoReleaseNotes() {
  loadingNotes.value = true;
  const results = await GithubService.getReleases(REPO.IM_QUERY_RUNNER);
...
function close() {
  if (releases.value.length) setLocalVersion("IMQueryRunner", releases.value[0]!.version);
